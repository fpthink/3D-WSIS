...
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
def read_s3dis_format_v2(raw_path, label_out=True):
#S3DIS specific
    objects = glob.glob(os.path.dirname(raw_path) + "/Annotations/*.txt")
    objects = sorted(objects) # 保证顺序不变
    ...
    for single_object in objects:
        ...
        # points = pd.read_csv(single_object, sep=' ', header=None).values # 使用 read_csv 读取数据会出问题
    ...
    xyz, rgb, room_labels, room_object_indices = data_label[:, :3], data_label[:, 3:6], data_label[:, 6], data_label[:, 7]

    xyz = xyz.astype('float32')
    rgb = rgb.astype('uint8')
    room_labels = room_labels.astype('uint8')
    room_object_indices = room_object_indices.astype('uint32')
