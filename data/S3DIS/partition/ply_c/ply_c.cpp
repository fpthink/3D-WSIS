...
typedef boost::tuple< std::vector< std::vector<float> >, std::vector< std::vector<uint8_t> >, 
                                    std::vector< std::vector<uint32_t> >, std::vector<std::vector<uint32_t> >, std::vector<uint32_t> > Custom_tuple;
...
{//converts to a python tuple
    static PyObject* convert(const Custom_tuple & c_tuple){
        ...
        PyObject * pyo5 = VecToArray32::convert(c_tuple.get<4>());
...
{//prune the point cloud xyz with a regular voxel grid
    ...
    // 2022.5.2 增加返回 point to voxel 的信息
    std::vector<uint32_t> point_to_voxel(n_ver, 0);
    ...
    for (std::size_t i_ver = 0; i_ver < n_ver; i_ver ++)
    {
        ...
        // point to voxel map
        point_to_voxel.at(i_ver) = vox_grid.get_index(bin_x, bin_y, bin_z);
    }
    ...
    return to_py_tuple::convert(Custom_tuple(pruned_xyz, pruned_rgb, pruned_labels, pruned_objects, point_to_voxel));
}
