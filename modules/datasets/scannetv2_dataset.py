...
class ScanNetV2Inst_spg(Dataset):
    def __init__(self, param, test_mode: bool=False):
                 
        # initialize dataset parameters
        self.logger = utils.derive_logger(__name__)         # __name__ == modules.datasets.scannetv2
        self.data_root = param.data_root                                        # data root path
        self.full_scale = list(map(int, ast.literal_eval(param.full_scale)))    # [128, 512]
        self.scale = param.scale                                                # 50
        self.max_npoint = param.max_npoint                                      # 250000
        self.with_elastic = param.with_elastic                                  # False

        self.task = param.task                                                  # "train"
        self.test_mode = test_mode                                              # False
        self.aug_flag = "train" in self.task
        
        self.debug = getattr(param, "debug", False)

        self.annotation_num = getattr(param, "annotation_num", 1)
    ...
    def weak_label_propagation(self, scene_name, sp_semantic_value, superpoint_pred_semantic, superpoint_affinity_matrix, iterations_num):
        ...
        CLASS_NUM = 20 # scannet v2 has 20 categories
...
def show_weak_label(scene_name, xyz, superpoint, point_instance_label_GT, graph, discribe):
    ...
    for v in graph.vs:
        if v['semantic_label'] != -100 and v['instance_label'] != -100 and v['semantic_label'] != 0 and v['semantic_label'] != 1: # 不显示 floor wall
            spId = v['v']
            instance_label = int(v['instance_label'])
            spMask = (superpoint == spId)
